...
**Idoneidad climática y disponibilidad real de hábitat para *Jubaea chilensis* (Palma chilena) mediante modelado de nicho ecológico e integración con MapBiomas Chile (cobertura de suelo y fuego) y WDPA/SNASPE (áreas protegidas).**
...
Este trabajo aplica el mismo pipeline de ensemble modeling (biomod2) ya utilizado para 19 especies del BGEN-UNAJ en Argentina y para *Mauritia flexuosa* en Perú, ahora sobre *Jubaea chilensis*, usando como insumo principal **MapBiomas Chile Colección 2 (2024)** —el producto de land cover exigido por el requisito de elegibilidad N.º 1 del reglamento del Premio— para responder si el hábitat climáticamente apto para la especie todavía existe como vegetación natural o ya fue perdido por conversión de uso de suelo. Las métricas de validación del modelo de nicho (AUCroc 0,948, TSS 0,741) provienen de validación aleatoria sobre datos con autocorrelación espacial; una prueba de robustez con *thinning* espacial real y validación cruzada por bloques espaciales (sección 7) arroja resultados más conservadores (TSS medio 0,460, con el algoritmo MAXNET generalizando notablemente mejor que Random Forest), lo que se documenta con total transparencia sin alterar los resultados operativos del estudio.

El resultado principal: el 77% del hábitat climáticamente apto sigue disponible como vegetación natural o restaurable, mientras que el 22,9% ya fue convertido a usos incompatibles. De ese hábitat disponible, solo el 12,6% está dentro de un área protegida —cruce realizado con **WDPA/SNASPE, fuente externa a MapBiomas**— y el 87,4% restante no tiene ninguna figura de protección legal.

Como control de robustez se repitió el cruce con la Colección 1 (2022), que arroja un resultado aparentemente más optimista; un análisis píxel a píxel (sección 6.4) muestra que el 95,3% de esa diferencia proviene de una reclasificación más granular de la clase ambigua "Mosaico Agropecuario" entre colecciones, no de conversión real de hábitat ocurrida entre 2022 y 2024 —un hallazgo metodológico relevante para cualquier estudio que combine series temporales de distintas colecciones de MapBiomas.

El cruce con el **Producto Fuego Colección 1** de MapBiomas Chile agrega un matiz crítico: ni siquiera la protección formal garantiza resguardo frente a incendios — un palmar protegido específicamente para la especie ("Palmar El Salto") perdió el 80% de su hábitat compatible al fuego entre 2013 y 2025.

Finalmente, una proyección a futuro con un ensamble de 6 modelos climáticos globales (CMIP6, SSP2-4.5, 2041-2060 — sección 6.8) muestra alta incertidumbre inter-modelo (mediana -0,2%, rango -46,2% a +17,0%) en vez de una contracción severa consensuada. Esa incertidumbre —no una cifra puntual— es el hallazgo: refuerza la urgencia de proteger ahora lo que ya es cierto (12,6% protegido, 80% de Palmar El Salto quemado), en vez de condicionar la estrategia de conservación a que se confirme un escenario climático específico.
