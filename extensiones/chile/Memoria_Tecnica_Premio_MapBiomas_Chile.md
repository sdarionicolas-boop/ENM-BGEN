...
Ninguna de estas limitaciones invalida los hallazgos centrales de este trabajo: la disponibilidad real de hábitat (sección 6.2), la brecha de protección (6.5), el riesgo de incendio (6.6) y la reducción proyectada a futuro (6.8) son consistentes entre sí, con la literatura, y con la verificación de sitios conocidos (6.1) — patrones que no dependen del valor exacto de TSS del modelo. Sí acotan la certeza con la que deben interpretarse las cifras puntuales, en particular las métricas de validación (sección 5.3, cuantificadas arriba bajo un esquema más riguroso) y el porcentaje exacto de contracción climática a 2050 (sección 6.8, con un único GCM).
