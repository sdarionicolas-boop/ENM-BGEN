# =============================================================================
#
#   BLOQUE 7 (Chile): Cruce con MapBiomas Chile Colección 1 (2022)
...
#   de MapBiomas Chile Colección 1 (2000-2022).
#
#   Pregunta:
...
#
#   Clasificación de uso del suelo (Chile Col. 1, códigos verificados
#   contra la leyenda oficial de Colección 2 y contrastados con los
#   códigos realmente presentes en la AOI de Jubaea chilensis):
#
...
# -----------------------------------------------------------------------------
# Leyenda MapBiomas Chile – Colección 1 (2022)
# Fuente: chile.mapbiomas.org/codigos-de-la-leyenda/
# Códigos verificados por inspección directa (freq()) del raster recortado
# a la AOI de Jubaea chilensis (29,9°S-35,3°S).
# -----------------------------------------------------------------------------
...
cat("\nCargando MapBiomas Chile 2022...\n")
mb_raw <- rast(file.path(VAR_DIR, "chile_coverage_2022.tif"))
...
for (i in seq_along(tifs_clase)) {
  ...
  tryCatch({
    ...
    writeRaster(cruce, file.path(MB_DIR, paste0(sp_name, "_cruce.tif")), overwrite = TRUE)
    ...
    write_csv(resumen_sp, file.path(MB_DIR, paste0(sp_name, "_cruce_resumen.csv")))
...
tifs_cruce <- list.files(MB_DIR, pattern = "_cruce.tif", full.names = TRUE)
...
m_cruce <- m_cruce %>%
  ...
  addLegend(
    ...
    title = "Idoneidad × Uso del suelo<br><small>MapBiomas Chile Col. 1 (2022)</small>", opacity = 0.9
  ) %>%
  addScaleBar(position = "bottomleft") %>%
  addMiniMap(toggleDisplay = TRUE)

Sys.setenv(RSTUDIO_PANDOC = "C:/Users/sdari/AppData/Local/Programs/Quarto/bin/tools")
ruta_html_cruce <- file.path(MB_DIR, "mapa_interactivo_cruce_chile.html")
